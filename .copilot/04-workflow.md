...
- [ ] **A1. Congelar a base verde.** Rodar `pnpm build` e `pnpm --filter @radinho/web test`
...
- [ ] **A2. Decidir a árvore suja.** Revisar o diff de `src/lib/local/detalheDaFaixa.ts`,
...
- [ ] **A3. Commit de base**, isolado das correções. _Aceite:_ `git status` limpo.
- [ ] **A4. Arnês de reprodução.** Criar `src/lib/audio/__tests__/arnesDeAudio.ts` com um
...
- [ ] **B1. Teste que falha.** `src/lib/audio/__tests__/tokenDeGeracao.test.ts`: 5 `playTrack`
      em sequência com `play()` rejeitando `AbortError` no meio.
      _Aceite:_ o teste falha na base (player fica parado / promessa rejeita).
- [ ] **B2. Token de geração.** Em `src/lib/audio/AudioEngine.ts`, `seq` incremental por
...
- [ ] **B3. Reconciliação de intenção.** Em `src/stores/playerStore.ts`, guardar a intenção
...
- [ ] **C1. Teste que falha.** `src/lib/audio/__tests__/duracao.test.ts` com `duration`
      `NaN`, `Infinity` e `0`, e faixa com `durationMs` conhecido.
      _Aceite:_ falha na base ao menos no caso `durationMs` conhecido ignorado.
- [ ] **C2. Duração persistida como fonte primária.** Ler `durationMs` da faixa
...
- [ ] **C3. Barreira no MediaSession.** Em `src/lib/audio/mediaSession.ts`, nada não-finito,
...
- [ ] **D1. Teste que falha.** `src/stores/__tests__/filaNaoPara.test.ts`: faixa 2 responde
      403 (cofre podado) e faixa 3 estagna sem evento.
      _Aceite:_ falha na base (fila para / spinner eterno).
- [ ] **D2. Erro marca e avança.** Em `playerStore.ts` + `src/lib/local/faixasQueFalharam.ts`,
...
- [ ] **D3. Watchdog de estagnação.** Timer único no `playerStore` (sem relógio repetido em
...
- [ ] **E1. Auditoria do `new Howl`.** Confirmar `html5: true` em `AudioEngine.ts` e mapear
...
- [ ] **E2. Web Audio só com a página visível.** Conectar o grafo (EQ/espectro) só com
...
- [ ] **E3. Um dono do foco de áudio** (um slot ativo por vez). _Aceite:_ teste afirma no
      máximo 1 elemento com `paused === false` após 10 trocas.

## Bloco F — RF6: offline real

- [ ] **F1. Teste que falha.** `src/lib/local/__tests__/tocaOffline.test.ts`: faixa baixada,
...
- [ ] **F2. Resolver fonte local antes da rede.** Ordem local (Cache Storage/IndexedDB) →
...
- [ ] **F3. SW não cacheia áudio.** Confirmar em `vite.config.ts` que `runtimeCaching` não pega áudio
      (sem Range/206). _Aceite:_ nenhuma regra nova de áudio; restrição comentada.

## Bloco G — RNF3/RNF4: otimização

- [ ] **G1. Alças de blob com dono.** Revogar em troca de faixa, `ended` e unmount usando
...
- [ ] **G2. Remover `wavesurfer.js`.** Tirar de `apps/web/package.json` e rodar `pnpm install`.
      _Aceite:_ `grep -r wavesurfer apps/web/src` sem resultado; `pnpm build` verde.
- [ ] **G3. `hls.js` fora do caminho crítico.** Confirmar `import type` (ou virar `import()`
      dinâmico). _Aceite:_ nenhum chunk inicial com `hls` em `apps/web/dist/assets`.

## Bloco H — segurança, verificação visual e deploy

- [ ] **H1. Varredura de segredo no diff** (`git diff`), `.env.example` atualizado se entrar
      variável nova. _Aceite:_ nenhum token/chave literal no diff (RNF7).
- [ ] **H2. Portões finais.** `pnpm build` + `pnpm test` na raiz.
      _Aceite:_ ambos código 0 (ou só as falhas de base registradas em A1).
- [ ] **H3. Verificação visual.** `pnpm --filter @radinho/web e2e` (`e2e/app.spec.ts`) e uma
...
- [ ] **H6. Riscos não entregues** em `.copilot/05-execucao.md`: 10 vulns de dependência,
